...
- **OCP Home** exposes its API only through the operator's in-process proxy. The [tailnet policy](../../../ocp-home/system/tailscale/tailnet-policy.json) grants TCP 443 to `tag:ocp-home-api` from `autogroup:member` and `autogroup:tagged`, mapped to `tailnet-readers`. The OCP GPU [tailnet policy](tailnet-policy.json) additionally maps `tag:ocp-gpu-devspaces` to `tailnet-admins`. There are no tokens or client certificates.
...
1. Add the entries from [tailnet-policy.json](tailnet-policy.json) to the tailnet policy. The policy lives in the private `morey-tech/homelab-private` repo at `tailscale/policy.hujson`. PRs there run validation and post a diff against the live policy; merging to `main` applies it through GitHub Actions. Don't edit the policy in the admin console, because the next merge overwrites it. The fragment defines `tag:ocp-gpu-operator`, lets it own `tag:ocp-gpu-devspaces`, and grants that proxy tag TCP 443 to `tag:ocp-home-api` with Kubernetes group `tailnet-admins`. [tailnet-admins.yaml](../../../ocp-home/system/tailscale/tailnet-admins.yaml) on OCP Home binds that group to `cluster-admin`. Argo CD does not apply this file; it mirrors the relevant part of the policy for reference.
